...
function render_bulk_edit_products_page() {
	...
	echo '<h3><span class="dashicons dashicons-search"></span> ' . esc_html__( 'Search and Filter', 'coding-bunny-bulk-edit' ) . '</h3>';
...
// Sort selector
echo '<label for="order_by" class="filter-label">' . esc_html__( 'Sort by Name', 'coding-bunny-bulk-edit' ) . '</label>';
...
echo '<label for="shipping_class" class="filter-label">' . esc_html__( 'Filter by Shipping Class', 'coding-bunny-bulk-edit' ) . '</label>';
echo '<select name="shipping_class" id="shipping_class" class="filter-select" ' . ( ! $licence_active ? 'disabled' : '' ) . '>';
echo '<option value="">' . esc_html__( 'All', 'coding-bunny-bulk-edit' ) . '</option>';
...
echo '<h3><span class="dashicons dashicons-editor-table"></span> ' . esc_html__( 'Products Table', 'coding-bunny-bulk-edit' ) . '</h3>';
...
echo '<div class="product-table-container" style="overflow-x: auto;">';
echo '<table class="wp-list-table widefat fixed striped" style="min-width: 2400px;">';
echo '<thead><tr>
	<th style="width: 10px;"></th><th style="width: 10px;"></th>
<th style="width: 10px;"><input type="checkbox" id="select-all"></th>
<th style="width: 100px;">' . esc_html__( 'Product name', 'coding-bunny-bulk-edit' ) . '</th>
<th style="width: 60px;">' . esc_html__( 'Regular Price', 'coding-bunny-bulk-edit' ) . ' (' . esc_html( $currency_symbol ) . ')</th>
<th style="width: 60px;">' . esc_html__( 'Sale Price', 'coding-bunny-bulk-edit' ) . ' (' . esc_html( $currency_symbol ) . ')</th>
<th style="width: 60px;">' . esc_html__( 'Manage Stock', 'coding-bunny-bulk-edit' ) . '</th>
<th style="width: 60px;">' . esc_html__( 'Stock Quantity', 'coding-bunny-bulk-edit' ) . '</th>
<th style="width: 60px;">' . esc_html__( 'SKU', 'coding-bunny-bulk-edit' ) . '</th>
<th style="width: 60px;">' . esc_html__( 'Weight', 'coding-bunny-bulk-edit' ) . ' (' . esc_html( $weight_unit ) . ')</th>
<th style="width: 60px;">' . esc_html__( 'Lenght', 'coding-bunny-bulk-edit' ) . ' (' . esc_html( $dimension_unit ) . ')</th>
<th style="width: 60px;">' . esc_html__( 'Width', 'coding-bunny-bulk-edit' ) . ' (' . esc_html( $dimension_unit ) . ')</th>
<th style="width: 60px;">' . esc_html__( 'Height', 'coding-bunny-bulk-edit' ) . ' (' . esc_html( $dimension_unit ) . ')</th>
<th style="width: 80px;">' . esc_html__( 'Shipping Class', 'coding-bunny-bulk-edit' ) . '</th>
<th style="width: 60px;">' . esc_html__( 'Reviews', 'coding-bunny-bulk-edit' ) . '</th>
<th style="width: 60px;">' . esc_html__( 'Status', 'coding-bunny-bulk-edit' ) . '</th>
<th style="width: 40px;">' . esc_html__( 'Menu Order', 'coding-bunny-bulk-edit' ) . '</th>	
<th style="width: 40px;">ID</th></tr></thead>';
echo '<tbody>';
...
if ( $products->have_posts() ) {
	...
	if ( $product->is_type( 'simple' ) || $product->is_type( 'variable' ) ) {
		...
		$selected_option_rew = $enable_reviews ? '1' : '0';
		...
		echo '<select name="shipping_class[' . esc_attr($product_id) . ']" style="width: 100%;" ' . ( ! $licence_active ? 'disabled' : '' ) . '>';
		echo '<option value="">' . esc_html__('No Shipping Class', 'coding-bunny-bulk-edit') . '</option>';
		foreach ($shipping_classes as $shipping_class) {
			$selected = selected($product_shipping_class, $shipping_class->term_id, false);
			echo '<option value="' . esc_attr($shipping_class->term_id) . '" ' . esc_attr($selected) . '>' . esc_html($shipping_class->name) . '</option>';
		}
		echo '</select>';
		echo '</td>';
		echo '<td><select name="enable_review[' . esc_attr( $product_id ) . ']" style="width: 100%;\" ' . ( ! $licence_active ? 'disabled' : '' ) . '>';
		echo '<option value="0"' . selected( $selected_option_rew, '0', false ) . '>' . esc_html__( 'Disable', 'coding-bunny-bulk-edit' ) . '</option>';
		echo '<option value="1"' . selected( $selected_option_rew, '1', false ) . '>' . esc_html__( 'Enable', 'coding-bunny-bulk-edit' ) . '</option>';
		echo '</select></td>';
		echo '<td>';
		...
		echo '</select>';
		echo '</td>';
		echo '<td><input type="number" step="1" name="menu_order[' . esc_attr( $product_id ) . ']" value="' . esc_attr( $menu_order ) . '" style="width: 100%;"></td>';
		...
		if ( $product->is_type( 'variable' ) ) {
			...
			foreach ( $variations as $variation ) {
				...
				echo '<td></td>';
				echo '<td><a href="' . esc_url( $variation_link ) . '" target="_blank" class="dashicons dashicons-visibility" title="' . esc_attr__( 'Display product', 'coding-bunny-bulk-edit' ) . '"></a></td>';
				...
				echo '<td>';
				$shipping_classes = WC()->shipping->get_shipping_classes();
				echo '<select name="shipping_class[' . esc_attr($variation_id) . ']" style="width: 100%;" ' . ( ! $licence_active ? 'disabled' : '' ) . '>';
				echo '<option value="">' . esc_html__('No Shipping Class', 'coding-bunny-bulk-edit') . '</option>';
				foreach ($shipping_classes as $shipping_class) {
					$selected = selected($variation_shipping_class, $shipping_class->term_id, false);
					echo '<option value="' . esc_attr($shipping_class->term_id) . '" ' . esc_attr($selected) . '>' . esc_html($shipping_class->name) . '</option>';
				}
				echo '</select>';
				echo '</td>';
...
echo '</tbody></table>';
echo '</div>';
echo '<input type="submit" name="update_products" value="' . esc_html__( 'Update Products Table', 'coding-bunny-bulk-edit' ) . '" class="button button-primary" style="margin-top: 20px; margin-bottom: 20px;">';
echo '<hr>';
echo '<h3><span class="dashicons dashicons-cart"></span> ' . esc_html__( 'Bulk Edit', 'coding-bunny-bulk-edit' ) . '</h3>';
...
echo '</div>';
echo '<div class="bulk-edit-container">';
echo '<label for="bulk_shipping_class" class="bulk-label">' . esc_html__( 'Shipping Class', 'coding-bunny-bulk-edit' ) . '</label>';
echo '<select name="bulk_shipping_class" id="bulk_shipping_class" class="bulk-select" ' . ( ! $licence_active ? 'disabled' : '' ) . '>';
...
echo '</select>';
echo '<label for="bulk_enable_review" class="bulk-label">' . esc_html__( 'Review', 'coding-bunny-bulk-edit' ) . '</label>';
echo '<select name="bulk_enable_review" id="bulk_enable_review" class="bulk-select" ' . ( ! $licence_active ? 'disabled' : '' ) . '>';
echo '<option value="no_change">' . esc_html__( 'No Change', 'coding-bunny-bulk-edit' ) . '</option>';
echo '<option value="yes">' . esc_html__( 'Enable', 'coding-bunny-bulk-edit' ) . '</option>';
echo '<option value="no">' . esc_html__( 'Disable', 'coding-bunny-bulk-edit' ) . '</option>';
echo '</select>';
echo '<label for="bulk_post_status" class="bulk-label">' . esc_html__( 'Status', 'coding-bunny-bulk-edit' ) . '</label>';
echo '<select name="bulk_post_status" id="bulk_post_status" class="bulk-select" ' . ( ! $licence_active ? 'disabled' : '' ) . '>';
echo '<option value="no_change">' . esc_html__( 'No Change', 'coding-bunny-bulk-edit' ) . '</option>';
...
function coding_bunny_add_product_column($columns) {
$new_columns = [];
foreach ($columns as $key => $value) {
	...
	if ($key === 'title') {
		...
		$new_columns['shipping_class'] = __('Shipping Class', 'coding-bunny-bulk-edit');
		$new_columns['enable_review'] = __('Enable Review', 'coding-bunny-bulk-edit');
		$new_columns['menu_order'] = __('Menu Order', 'coding-bunny-bulk-edit');
	}
}
return $new_columns;
}
add_filter('manage_edit-product_columns', 'coding_bunny_add_product_column', 15);

function coding_bunny_show_product_column($column, $post_id) {
...
if ($column === 'product_sku') {
	...
	$manage_stock = $product->get_manage_stock();
	$selected_option = $manage_stock ? '1' : '0';
	echo '<select name="manage_stock[' . esc_attr($post_id) . ']" style="width: 100%;">';
	echo '<option value="0"' . selected($selected_option, '0', false) . '>' . esc_html__('NO', 'coding-bunny-bulk-edit') . '</option>';
	echo '<option value="1"' . selected($selected_option, '1', false) . '>' . esc_html__('YES', 'coding-bunny-bulk-edit') . '</option>';
	echo '</select>';
...
} elseif ($column === 'shipping_class') {
	$shipping_classes = WC()->shipping->get_shipping_classes();
	$product_shipping_class = $product->get_shipping_class_id();
	echo '<select name="shipping_class[' . esc_attr($post_id) . ']" style="width: 100%;">';
	echo '<option value="">' . esc_html__('No Shipping Class', 'coding-bunny-bulk-edit') . '</option>';
	foreach ($shipping_classes as $shipping_class) {
		$selected = selected($product_shipping_class, $shipping_class->term_id, false);
		echo '<option value="' . esc_attr($shipping_class->term_id) . '" ' . esc_attr($selected) . '>' . esc_html($shipping_class->name) . '</option>';
	...
	echo '</select>';
} elseif ($column === 'enable_review') {
	$review_enabled = $product->get_reviews_allowed() ? 'yes' : 'no';
	echo '<select name="enable_review[' . esc_attr($post_id) . ']" style="width: 100%;">';
	echo '<option value="yes"' . selected($review_enabled, 'yes', false) . '>' . esc_html__('Enable', 'coding-bunny-bulk-edit') . '</option>';
	echo '<option value="no"' . selected($review_enabled, 'no', false) . '>' . esc_html__('Disable', 'coding-bunny-bulk-edit') . '</option>';
	echo '</select>';
} elseif ($column === 'menu_order') {
	$menu_order = $product->get_menu_order();
	echo '<input type="number" step="1" name="menu_order[' . esc_attr($post_id) . ']" value="' . esc_attr($menu_order) . '" class="widefat" style="width: 100%;">';
}
}
add_action('manage_product_posts_custom_column', 'coding_bunny_show_product_column', 10, 2);
...
isset($_POST['regular_price']) && 
				...
				isset($_POST['manage_stock']) && 
					isset($_POST['weight']) && 
						isset($_POST['length']) && 
							isset($_POST['width']) && 
								isset($_POST['height']) &&
									isset($_POST['shipping_class']) &&
										isset($_POST['post_status']) &&
											isset($_POST['menu_order']) &&
												isset($_POST['enable_review'])
) {
	...
	$shipping_classes = wp_unslash($_POST['shipping_class']);
	$enable_reviews = wp_unslash($_POST['enable_review']);
	$menu_orders = array_map('intval', wp_unslash($_POST['menu_order']));
	...
	foreach ($regular_prices as $product_id => $regular_price) {
		...
		if ($product && $product->is_type('variable')) {
			...
			foreach ($variations as $variation) {
				$variation_id = $variation['variation_id'];
				if (isset($shipping_classes[$variation_id])) {
					$variation_shipping_class_id = intval($shipping_classes[$variation_id]);
					$variation_product = wc_get_product($variation_id);
					$variation_product->set_shipping_class_id($variation_shipping_class_id);
					...
					if ($product) {
						...
						if (isset($shipping_classes[$product_id])) {
							$shipping_class_id = intval($shipping_classes[$product_id]);
							$product->set_shipping_class_id($shipping_class_id);
						}

						if (isset($enable_reviews[$product_id])) {
							$review_enabled = $enable_reviews[$product_id] === '1' ? true : false;
							$product->set_reviews_allowed($review_enabled);
						}	

						if (isset($menu_orders[$product_id])) {
							$menu_order = $menu_orders[$product_id];
	...
	function update_bulk_products() {
		if (isset($_POST['bulk_update_products'])) {
			...
			$bulk_shipping_class = isset($_POST['bulk_shipping_class']) ? wp_unslash($_POST['bulk_shipping_class']) : 'no_change';
			$bulk_post_status = isset($_POST['bulk_post_status']) ? wp_unslash($_POST['bulk_post_status']) : 'no_change';
			$bulk_enable_review = isset($_POST['bulk_enable_review']) ? wp_unslash($_POST['bulk_enable_review']) : 'no_change';
			...
			if (!empty($_POST['selected_products']) && is_array($_POST['selected_products'])) {
				foreach ($_POST['selected_products'] as $product_id) {
					...
					if ($product) {
						...
						if ($bulk_shipping_class !== 'no_change') {
							$product->set_shipping_class_id($bulk_shipping_class);
						}

						if ($bulk_post_status !== 'no_change') {
							$product->set_status($bulk_post_status);
						}

						if ($bulk_enable_review !== 'no_change') {
							$review_enabled = $bulk_enable_review === 'yes' ? true : false;
							$product->set_reviews_allowed($review_enabled);
						}
